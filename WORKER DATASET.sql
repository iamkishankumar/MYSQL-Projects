...
#Q-45 Write an SQL query to add middle name column after first_name column
alter table worker add column MIDDLE_NAME varchar(30) after first_name;


#Q-46 Write an SQL query to delete a single column
alter table worker drop column middle_name;


#Q-47 Write an SQL query to delete a single row in a table 
delete from worker where worker_id = 8;


#Q-48 Write an SQL query to find null rows
select * from worker where worker_id is null;


#Q-49 Write an SQL query to show data where S include in his/her name
SELECT * FROM worker WHERE first_name REGEXP "S";
...
#Q-51 Write an SQL query to create a function which is return the level of department

DELIMITER //
create function performance(department char(25) ) returns varchar(30)
begin
	declare level varchar(30)
	if department = "HR" then set level = "1st level";
    elseif department = "Admin" then set level = "2nd level";
	elseif department = "Account" then set level = "3rd level";
end if;
return(level)
end
DELIMITER //
